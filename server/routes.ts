...
export async function registerRoutes(app: Express): Promise<Server> {
  // Create video job endpoint (asynchronous processing for Autoscale)
  app.post("/api/video-jobs", async (req, res) => {
    try {
      ...
      console.log(`🎬 Starting asynchronous processing for job ${job.id} (speed optimized)`);
      
      // Start processing in background (non-blocking) with speed optimizations
      ...
      res.json({
        job_id: job.id,
        status: "queued",
        message: "High-speed video processing started. Poll /api/video-jobs/{job_id} for status updates.",
