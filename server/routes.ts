...
export async function registerRoutes(app: Express): Promise<Server> {
  // Create video job endpoint (synchronous processing for VM deployment)
  app.post("/api/video-jobs", async (req, res) => {
    try {
      ...
      // Process video synchronously - wait for completion (VM has no timeout issues)
      try {
        ...
        // Send complete response with video URL
        const response = {
          job_id: job.id,
